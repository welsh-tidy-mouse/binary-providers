...
class DartSassBinaryProvider implements BinaryProviderInterface
{
    ...
    public function getBinaryFilenameFromDownloadedAsset(string $assetFileName, string $downloadDirPath): string
    {
        ...
        match ($ext) {
            ...
            default => throw new BinaryProviderException($this->getName(), 'Asset extension file can not be matched.'),
        };
        ...
        $this->filesystem->copy(
            Path::join($downloadDirPath, 'dart-sass', $targetBinaryFileName),
            Path::join($downloadDirPath, $targetBinaryFileName)
        );

        $this->filesystem->mirror(
            Path::join($downloadDirPath, 'dart-sass', 'src'),
            Path::join($downloadDirPath, 'src')
        );

        $this->filesystem->remove(Path::join($downloadDirPath, 'dart-sass'));
